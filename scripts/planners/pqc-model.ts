...
export type PqcOption = { id: string; title: string; description: string };
export type PqcQuestion = { id: string; eyebrow: string; question: string; context: string; options: readonly PqcOption[] };
...
] as const satisfies readonly PqcQuestion[];

const X = { le1: 1, mid: 5, ten: 10, long: 25 } as const;
const Zbase = { aggressive: 5, central: 8, conservative: 15, mandate: 7 } as const;
const Ybase = { y1: 1, y3: 3, y5: 5, undecided: 4 } as const;
const capAdj = { constrained: 2, moderate: 1, strong: 0, dedicated: -1 } as const;
const agiAdj = { hardcoded: 2, config: 1, abstracted: 0, agile: -1 } as const;
const depAdj = { low: 0, some: 1, significant: 2, pervasive: 3 } as const;
...
const surfaceToDomain = { tls: 'tls', data: 'data', code: 'code', identity: 'identity' } as const;
...
export function createPqcPlan(answers: AnswerMap) {
  ...
  const x = X[answers.shelf as keyof typeof X] ?? 5;
  let z = Zbase[answers.horizon as keyof typeof Zbase] ?? 8;
  let y = Ybase[answers.program as keyof typeof Ybase] ?? 4;
  y += capAdj[answers.capacity as keyof typeof capAdj] ?? 0;
  y += agiAdj[answers.agility as keyof typeof agiAdj] ?? 0;
  y += depAdj[answers.dependency as keyof typeof depAdj] ?? 0;
  y = Math.max(1, y);
  ...
  const sd = surfaceToDomain[answers.surface as keyof typeof surfaceToDomain];
  const cd = crownToDomain[answers.crown as keyof typeof crownToDomain];
  ...
  return {
    ...
    summary: `${band} urgency · ${complianceLabel[answers.compliance as keyof typeof complianceLabel] ?? 'risk-driven'} · data shelf-life ${x}y, migration ~${y}y, quantum horizon ${z}y`,
    xLabel: `${x}-year data shelf-life`, yLabel: `~${y}-year migration`, zLabel: `${z}-year quantum horizon`,
    complianceLabel: complianceLabel[answers.compliance as keyof typeof complianceLabel] ?? 'Risk-driven',
