...
const zoneCounts = { one: 1, three: 3, six: 6, twelve: 12 }         ;
const assetCounts = { small: 500, medium: 2500, large: 10000, xlarge: 25000 }         ;
const retentionDays = { seven: 7, thirty: 30, ninety: 90, year: 365 }         ;
const estateLabels = { it: 'Enterprise IT estate', ot: 'Operational technology estate', cloud: 'Cloud and SaaS estate', hybrid: 'Hybrid IT, OT, cloud and edge estate' }         ;
const collectionLabels = { import: 'Approved imports only', passive: 'Passive network evidence', cautious: 'Bounded active discovery', combined: 'Imports, passive and approved discovery' }         ;
const connectivityLabels = { private: 'Private routed connectivity', outbound: 'Outbound-only TLS 443', window: 'Controlled transfer window', airgap: 'Fully offline signed transfer' }         ;
const sourceLabels = { repo: 'One repository', application: 'One application', portfolio: 'Application portfolio', restricted: 'Restricted source enclave' }         ;
const responseLabels = { investigate: 'Investigate and hand off', rehearse: 'Investigate and rehearse', lab: 'Governed lab execution', adapter: 'Approved production adapter' }         ;
const inferenceLabels = { external: 'Approved external gateway', local: 'One local inference runtime', dual: 'Dual local inference runtimes', offline: 'Offline local inference' }         ;
...
export function createRecommendation(answers           ) {
  ...
  const collectors = zoneCounts[answers.networks                           ] ?? 1;
  const assets = assetCounts[answers.assets                            ] ?? 500;
  const days = retentionDays[answers.retention                              ] ?? 30;
  ...
  return {
    ...
    estateLabel: estateLabels[answers.estate                             ] ?? estateLabels.it,
    ...
    collectionLabel: collectionLabels[answers.collection                                 ] ?? collectionLabels.import,
    connectivityLabel: connectivityLabels[answers.connectivity                                   ] ?? connectivityLabels.private,
    sourceLabel: sourceLabels[answers.source                             ] ?? sourceLabels.repo,
    responseLabel: responseLabels[answers.response                               ] ?? responseLabels.investigate,
    inferenceLabel: inferenceLabels[answers.inference                                ] ?? inferenceLabels.external,
    ...
    sourceWorkers,
    environmentPlan: environmentPlans[answers.estate                                 ] ?? environmentPlans.it,
    flow: connectivityPlans[answers.connectivity                                  ] ?? connectivityPlans.private,
    collectionMode: `${ot && !active ? 'OT passive-first. ' : ''}${collectionPlans[answers.collection                                ] ?? collectionPlans.import}`,
    sourcePlan: sourcePlans[answers.source                            ] ?? sourcePlans.repo,
    responseMode: responsePlans[answers.response                              ] ?? responsePlans.investigate,
    inferencePlan: inferencePlans[answers.inference                               ] ?? inferencePlans.external,
    software: [
      ...
      { owner: answers.inference === 'external' ? 'customer' : 'vendor', label: 'Inference runtime', text: inferencePlans[answers.inference                               ] ?? inferencePlans.external },
      ...
      { owner: 'vendor', label: 'Agentic SOAR', text: responsePlans[answers.response                              ] },
